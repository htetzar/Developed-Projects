...
namespace GalaxySignage.Models
{
    public class FilesStatus
    {
        ...
        private void SetValues(string fileName, int fileLength, string fullPath)
        {
            ...
            if (fileSize > 3 || !IsImage(ext)) thumbnail_url = File.Exists(
                Path.ChangeExtension(
                fullPath.Replace(
                ConfigurationManager.AppSettings["VideoFolder"]
                , ConfigurationManager.AppSettings["ThumbnailFolder"])  
                , ConfigurationManager.AppSettings["ThumbnailImageExtension"]))

                ? @"/" + ConfigurationManager.AppSettings["ThumbnailFolder"]
                + @"/" + Path.GetFileNameWithoutExtension(fullPath)
                + ConfigurationManager.AppSettings["ThumbnailImageExtension"]

                : "/Content/img/video.png";
